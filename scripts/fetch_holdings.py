...
ARK_BASE = (
    "https://assets.ark-funds.com/"
    "fund-documents/funds-etf-csv"
)
...
ARKQ_URLS = [
    ...
    f"{ARK_BASE}/"
    "ARK_AUTONOMOUS_TECHNOLOGY_%26_ROBOTICS_ETF_ARKQ_HOLDINGS.csv",

    f"{ARK_BASE}/"
    "ARK_AUTONOMOUS_TECH_%26_ROBOTICS_ETF_ARKQ_HOLDINGS.csv",
...
TEMA_ETFS = {
    ...
    "NASA":
        "https://temaetfs.com/hubfs/Website/"
        "Holdings/NASA-holdings.csv",
...
def _clean_and_dedup(df, etf_ticker):
    """
    Clean weight column, tag etf/date,
    and deduplicate holdings.
    """
    ...
    if len(result) < before:
        ...
        print(
            f"  [{etf_ticker}] removed "
            f"{before - len(result)} duplicate rows"
        )

    print(
        f"  [{etf_ticker}] "
        f"{len(result)} holdings — top: "
        f"{result['ticker'].head(3).tolist()}"
    )
...
def fetch_ark_with_fallback(
    etf_ticker: str,
    urls: list
):

    """
    Try multiple ARK URLs.
    """
    ...
    print(
        f"  [{etf_ticker}] "
        f"all variants failed — skipping"
    )
...
def inspect_ishares_html(
    etf_ticker: str,
    html: str
):
    """
    Inspect an HTML response to determine whether it contains
    embedded holdings data or references to another API endpoint.

    This function does not assume that the HTML contains holdings.
    It only reports useful clues.
    """
    ...
    print(
        f"  [{etf_ticker}] "
        f"inspecting HTML for embedded "
        f"holdings/API clues..."
    )
    ...
    relative_endpoints = re.findall(
        r'["\']([^"\']*'
        r'(?:ajax|api|holdings|portfolio)'
        r'[^"\']*)["\']',
        html,
        flags=re.IGNORECASE,
    )
    ...
    try:
        ...
        print(
            f"  [{etf_ticker}] "
            f"could not save HTML debug file: "
            f"{e}"
        )
...
def generate_multi_etf_signals():
    ...
    result = (
        supabase
        .table("holding_changes")
        .select(
            "ticker, etf, change_type"
        )
        .eq(
            "change_type",
            "accumulation"
...
def run():
    ...
    print(
        "\nFetching ARKQ "
        "(ARK, multi-URL fallback)..."
    )

    if save_etf(
        "ARKQ",
        fetch_ark_with_fallback(
            "ARKQ",
            ARKQ_URLS
        )
    ):
